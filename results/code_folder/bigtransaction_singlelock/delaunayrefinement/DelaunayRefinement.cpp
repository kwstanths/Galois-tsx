 ...
 */
//#define GALOIS_DEFINE_RTM
#define GALOIS_USE_HTM
//#define GALOIS_MY_RTM
#define RTM_GATHER_STATS
//#define HTM_DEBUG
#define number_of_aborts 25
...
struct Process {
	...
	void operator()(GNode item, Galois::UserContext<GNode>& ctx) {
		...
		if (Version == detDisjoint) {
			...
			if (transaction_start(&fallback_lock,number_of_aborts)){
				cav.initialize(item);
				cav.build();
				cav.computePost();
...
int main(int argc, char** argv) {
	...
	pthread_spin_init(&fallback_lock, 0);
	...
	pthread_spin_destroy(&fallback_lock);

#ifdef RTM_GATHER_STATS
	ReportStat& stats = ReportStat::getInstance();
	std::cout << "\tConflict aborts: " << stats.getConflictAborts() << std::endl;
	std::cout << "\tCapacity aborts: " << stats.getCapacityAborts() << std::endl;
	std::cout << "\tRetry aborts: " << stats.getRetryAborts() << std::endl;
	std::cout << "\tExplicit aborts: " << stats.getExplicitAborts() << std::endl;
	std::cout << "\tDebug aborts: " << stats.getDebugAborts() << std::endl;
	std::cout << "\tNested aborts: " << stats.getNestedAborts() << std::endl;
	std::cout << "\tTotal aborts: " << stats.getTotalAborts() << std::endl;
#endif
