...
struct FindMatching {
	...
	template<typename ContextTy>
		void operator()(NodeWrapper * nodeA, ContextTy& lwl) {
			...
			bool contains_A = tree->contains(*nodeA);
			...
			if (contains_A) {
				...
				transaction_start(&fallback_lock, number_of_aborts);
				bool contains_B = tree->contains(*nodeB);
				transaction_end(&fallback_lock);

				if (nodeB != NULL && contains_B) {
					transaction_start(&fallback_lock, number_of_aborts);
					...
					if (nodeBMatch!=NULL ){
						...
						transaction_start(&fallback_lock, number_of_aborts);
						bool equals_1 = nodeA->equals(*nodeBMatch);
						transaction_end(&fallback_lock);

						transaction_start(&fallback_lock, number_of_aborts);
						bool equals_2 = nodeA->equals(*nodeB);	
						transaction_end(&fallback_lock);
						
						if(equals_1 && equals_2 ==false) {
...
int main(int argc, char ** argv){
	...
	number_of_aborts=std::stoi(getenv("RTM_NUMBER_OF_ABORTS"));
