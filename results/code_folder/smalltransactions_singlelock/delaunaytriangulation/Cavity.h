...
class Cavity: private boost::noncopyable {
	...
	struct InCircumcenter {
		...
		bool operator()(const GNode& n) const {
			Element& e = graph.getData(n, Galois::MethodFlag::NONE);
	...
	void findOutside() {
		for (typename Searcher<Alloc>::GNodeVector::iterator ii = searcher.inside.begin(),
			...
			validation_vector.push_back(*ii);
			for (Graph::edge_iterator jj = graph.edge_begin(*ii, Galois::MethodFlag::NONE),
				...
				int index = graph.getEdgeData(graph.findEdge(n, *ii, Galois::MethodFlag::NONE));
				...
				p2->get(Galois::MethodFlag::CHECK_CONFLICT);
				p3->get(Galois::MethodFlag::CHECK_CONFLICT);
			}
		}
	}
	...
	void addElements() {
		...
		for (typename GNodeIntPairVector::iterator ii = outside.begin(),
			...
			graph.addNode(newNode, Galois::MethodFlag::NONE);

			point->addElement(newNode);
			p2->addElement(newNode);
			p3->addElement(newNode);

			graph.getEdgeData(graph.addEdge(newNode, n, Galois::MethodFlag::NONE)) = 1;
			graph.getEdgeData(graph.addEdge(n, newNode, Galois::MethodFlag::NONE)) = index;
		...
		for (unsigned i = 0; i < newNodes.size(); ++i) {
			...
			const Element& e1 = graph.getData(n1, Galois::MethodFlag::NONE);
			for (unsigned j = i + 1; j < newNodes.size(); ++j) {
				if (i != j) {
					const GNode& n2 = newNodes[j];
					const Element& e2 = graph.getData(n2, Galois::MethodFlag::NONE);
					...
					for (int x = 2; x >= 1; --x) {
						for (int y = 2; y >= 1; --y) {
							if (e1.getPoint(x) == e2.getPoint(y)) {
								...
								graph.getEdgeData(graph.addEdge(n1, n2, Galois::MethodFlag::NONE)) = indexForNewNode;
								graph.getEdgeData(graph.addEdge(n2, n1, Galois::MethodFlag::NONE)) = indexForNode;
							}
						}
					}
				}
